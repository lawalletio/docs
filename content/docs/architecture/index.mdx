---
title: "Architecture"
description: "Three independent containerized services with no shared infrastructure."
---

## Overview

LaWallet NWC is composed of **two fully independent, containerized services** plus an external courtesy wallet provider ([lncurl](https://github.com/rolznz/lncurl)). Each service runs in its own Docker container with its own database/storage. There is no shared infrastructure between services. They communicate exclusively via HTTP APIs and WebSocket events.
...
| External | [lncurl](https://github.com/rolznz/lncurl) (Courtesy NWC) | — | Managed by lncurl |
...
- No dependency on Next.js app database or courtesy NWC (lncurl)
...
Temporary NWC wallets provisioned via [**lncurl**](https://github.com/rolznz/lncurl) ([lncurl.lol](https://lncurl.lol)).

- Single HTTP POST to provision a new NWC wallet
- Powered by Alby Hub — battle-tested Lightning infrastructure
- No custom proxy container needed — lncurl is an external service
- Self-hostable: run your own lncurl instance for full sovereignty
- Pay-as-you-go: 1 sat/hour per wallet, auto-deleted when empty
...
|----------|------------|-------|
| Vercel | web only | Listener deployed separately, lncurl is external |
| Netlify | web only | Same as Vercel |
| Umbrel | web + listener | Full app store package, lncurl is external |
| Start9 | web + listener | Embassy package, lncurl is external |
| Docker Compose | web + listener | Independent containers + reverse proxy |
